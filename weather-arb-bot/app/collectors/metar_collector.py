...
class MetarCollector(BaseCollector):
    ...
    async def collect(self, icao: str) -> Optional[dict]:
        try:
            resp = await self._get(AVWX_METAR_URL, params={"ids": icao, "format": "json"})
            data = resp.json()
            if not data:
                return None
    ...
    async def collect_and_store(self, icao: str, db: AsyncSession) -> Optional[dict]:
        ...
        stmt = (
            ...
            .values(icao=icao, observed_at=parsed["observed_at"], temperature_f=parsed.get("temperature_f"), dew_point_f=parsed.get("dew_point_f"), humidity_pct=parsed.get("humidity_pct"), wind_direction=parsed.get("wind_direction"), wind_speed_kt=parsed.get("wind_speed_kt"), wind_gust_kt=parsed.get("wind_gust_kt"), pressure_hg=parsed.get("pressure_hg"), visibility_sm=parsed.get("visibility_sm"), conditions=parsed.get("conditions"), raw_metar=parsed.get("raw_metar"))
