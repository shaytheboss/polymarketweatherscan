...
Run as: python -m app.bot.telegram_bot
"""
import asyncio
import logging
import signal
...
async def send_opportunity_alert(opportunity, db) -> None:
    ...
    users_result = await db.execute(
        select(TelegramUser).where(
            TelegramUser.min_confidence <= opportunity.confidence_score
        )
    )
    ...
    for user in users:
        # Check if user watches this city
...
async def main():
    ...
    if settings.is_production:
        # Webhook mode for production (Railway provides HTTPS)
